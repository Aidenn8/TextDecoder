...
//===========\\
  ...
  final static int TREE_TYPE = TREE_TYPE_FROM_TEXT;//TREE_TYPE_STD;
  
  // File selections. CHANGE AS DESIRED.
  final static String TREE_F = HOBBIT_F;   // File to build tree from
  final static String ENCODE_F = HOBBIT_F;//CAGED_BIRD_F; // File to encode
  final static String DECODE_F = ENCODE_OUT_F;    // File to decode
  ...
  public static void main(String[] args) {
    ...
    System.out.println(encodeFileChars.length);
    ...
    if (decodeFileChars == null) {
      ...
      System.out.println("How many charaters in the passage?");
  ...
  static String encode(Map<Character,String> bitStrings, char[] text) {
    ...
    //go through char[]
    //for each character use the map to encode it
    //add to output
    for(char e : text) { //go through text
    	output.append(bitStrings.get(e)); //add on bitcode for the char
    }
